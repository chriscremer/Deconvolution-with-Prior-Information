...
if __name__ == "__main__":
		...
		for var in x_values:
			...
			for model in range(len(models)):
				...
				if models[model] == 'Difi':
					print models[model]
					decomposer = Difi(n_components=n_comps, tol=tol, lambda1=var)
	...
	# x_values = [math.log10(x) for x in x_values]

	nice_plot.plot_lines_2graphs_errorbars3_logscale(x_values, Z_error_avg, Z_error_std, models, 'Regularization Strength', 'Z Reconstruction Error', 'na', 1, 0, xlim=[1,10100])

	nice_plot.plot_lines_2graphs_errorbars3_logscale(x_values, W_error_avg, W_error_std, models, 'Regularization Strength', 'W Reconstruction Error', 'lambda_plot.png', 0, 1, xlim=[1,10100])

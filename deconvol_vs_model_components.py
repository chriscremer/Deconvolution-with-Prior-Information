...
def main():
	...
	for iteration in range(numb_of_iterations):
		...
		numb_components = [5, 10, 15, 20, 25, 30, 35]
		for n_comp in numb_components:

			print 'Testing ' + str(n_comp)	+ ' components'	
			#Make data
			percent_hidden = 0.5
			numb_subpops = 20
			numb_feats = 1000
			...
			for init_type in range(len(init_types)):
				...
				for try1 in range(numb_of_iters_to_remove_local_minima):
					...
					#Initializing model
					TZ = mt.init_model(init_types[init_type], n_comp, X)
				...
				scores.append(np.linalg.norm(X - X_hat))
	...
	# pbc.plot_three_lines_with_text(scores_to_plot, scores_to_plot2, scores_to_plot3, numb_components, 'L2-Norm', 'Number of Components', '../plots/norm_vs_model_components.png', text)

...
var_list = [5,10,15,20]
# var_list = [3,5,7,9,15,20,30]
# var_list = [3,5,6,10,15]
#Data Parameters
n_samps = 50
# n_comps = 10
# p_of_zero=.7
...
for iter_ in range(n_iters):
	...
	for var in var_list:
		...
		subpops, fractions, X = mrsd.make_and_return(n_subpops=var, n_samps=n_samps, probability_of_zero=(1.-(4./var)), noise=noise1, n_outliers=n_outliers)
		...
		for m in range(len(models)):
			...
			if models[m] == 'Projection':
				print models[m] 
				W = sfw.optimize_w_using_L2_sum_to_1(X, Z)
				print W[0]
				# if samp % 3 == 0:
			...
			if models[m] == 'Projection_Fit_Prior':
				...
				W_nnls = sfw.optimize_w_using_L2_sum_to_1(X, Z)
				...
				for samp in range(len(fractions)):
					...
					non_zero_weights = []
					for i in range(len(W_nnls[samp])):
						if W_nnls[samp][i] > 0:
							non_zero_weights.append(W_nnls[samp][i])	

					fitted_weight_vector = [0.]*len(Z)
					indexes_of_values = np.argsort(W_nnls[samp])[::-1]
			...
			for samp in range(len(fractions)):
				...
				# if np.linalg.norm(fractions[samp] - W[samp]) < .16 and np.linalg.norm(fractions[samp] - W[samp]) > .13:
				# 	print '----'
				# 	print fractions[samp]
				# 	print W[samp]
				# 	print np.linalg.norm(fractions[samp] - W[samp])
				# 	print '----'
			...
			if models[m] == 'Exhaustive_Search' and var > 5:
				Avg_L2_error = 0.
